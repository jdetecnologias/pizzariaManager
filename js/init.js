$(document).ready(function() {
  ...
  (function(e) {
    "strict mode"
    if (document.querySelector("#visualizarPedidos") == null) {} else {
        
      $(document).on("click", "#voltar", function() {
          pagamentoView.inicio();
      });
      $(document).on("click", "#verPedidoTable .iniciarIntervalo", function() {
       pagamentoView.interval();
      });
      var pagamentoView = {
        interval:false,
        time:3000,
        setInterval:function(){
          this.interval = setInterval(verPedidos,this.time); 
        },
        stopInterval:function(){
          clearInterval(this.interval);
          this.interval = false;
        },
        viewPagamentoView: function(objClicado) {
      ...
      $(document).on("click", "#receber", function() {
        ...
        var valor = arredondarNumeros(parseFloat($("#aReceber").val()), 2);
          ...
          $.ajax({
            ...
            success: function(r) {
              ...
              if(r.valorPendente == 0){
                setTimeout(pagamentoView.inicio,5000);
      ...
      $(document).on("click", ".viewPedido", function() {
        pagamentoView.viewPagamentoView($(this));
      });
      $(document).on("click", "#abaAbertos #cancelarPedido", function() {
        pagamentoView.stopInterval();
